...
function showLoading(show=true) {
  ...
  if(show){
    ...
    if(accessToken&&!searchBusy) setConnection("sync","Actualizando");
  }
}
...
async function driveList(q, extra={}) {
  ...
  let pageCount=0;
  const maxPages=Number(extra.maxPages||0);
  const maxItems=Number(extra.maxItems||0);
  do {
